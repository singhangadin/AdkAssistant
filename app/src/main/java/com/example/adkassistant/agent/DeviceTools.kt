 ...
 * `@Param` descriptions are what the model actually reads to decide *when* and
 * *how* to call it, so write them for the model, not just for humans.
 *
 * The KSP processor reads these annotations at compile time and generates
 * `generatedTools()`, which we hand to the agent.
 */
class DeviceTools {
